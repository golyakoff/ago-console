...
describe("the identity with no operators row is two identities", () => {
  ...
  it("sends the identity to the setup form when the owner probe cannot answer", async () => {
    ...
    // Deliberately not the "sign-in failed" screen `resolveOperatorState`'s own failure produces.
    // That call decides between a queue and a form and guessing wrong strands somebody; this one
    // decides between a form and a read-only view, and its wrong answer is a form the server refuses
    // and a page that explains itself. Failing back to exactly where this state went before `12-04`
    // means an owner-endpoint outage cannot change any ordinary registrant's destination.
    expect(container.textContent).toContain("set up your site");
