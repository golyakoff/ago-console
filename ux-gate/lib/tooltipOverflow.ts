/**
 * `25-111`: measures whether an open `Tooltip` bubble (`src/components/Tooltip.tsx`) is rendered
 * fully inside the nearest ancestor box that would actually clip it, given the trigger `<button>`
 * that owns it.
 *
...
export function measureTooltipTriggerOverflow(trigger: HTMLElement): {
  ...
  overflowPx: number;
  bubbleRect: { left: number; right: number } | null;
  clipRect: { left: number; right: number } | null;
} {
  function findClippingRect(element: Element): { left: number; right: number } {
  ...
  if (bubble === null || bubble === undefined) {
    return { found: false, overflowLeftPx: 0, overflowRightPx: 0, overflowPx: 0, bubbleRect: null, clipRect: null };
  }
  ...
  return {
    ...
    overflowLeftPx,
    overflowRightPx,
    overflowPx: overflowLeftPx + overflowRightPx,
    bubbleRect: { left: bubbleRect.left, right: bubbleRect.right },
    clipRect,
  };
}
