...
export function measureContrastViolations(): ContrastResult {
  ...
  function findRenderedBackground(start: Element): { color: Rgba; source: string } {
    ...
    return { color: { r: 255, g: 255, b: 255, a: 1 }, source: "browser-default(unreachable in this app)" };
  }
  ...
  while (current) {
    ...
    const fontWeight = fontWeightRaw === "bold" ? 700 : fontWeightRaw === "normal" ? 400 : parseInt(fontWeightRaw, 10);
    const isLargeText =
      fontSizePx >= LARGE_TEXT_REGULAR_PX || (fontSizePx >= LARGE_TEXT_BOLD_PX && fontWeight >= BOLD_WEIGHT_THRESHOLD);
